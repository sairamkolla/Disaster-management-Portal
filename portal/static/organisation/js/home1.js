...
app.controller('myctrl', ['$scope', '$http', '$templateCache','$interval',
    function($scope, $http, $templateCache,$interval) {
        ...
        $scope.decision = function(id,i,x){
            var data = {
                disasterid:id,
                decision:i,
                userid: $scope.id
            };
            $http.post('/data/DecisionOrgs/',data).
            then(function(success){
              console.log(success);
              var index = $scope.disasters.indexOf(x);
                $scope.disasters[index].value = i;
                console.log($scope.disasters[index]);
            },function(error){
              console.log(error);
            });
